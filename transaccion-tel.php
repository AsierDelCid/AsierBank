...
<h1 align="center"><i class="fas fa-university"></i>&nbsp;&nbsp;&nbsp;Enviar Dinero&nbsp;&nbsp;&nbsp;<i class="fas fa-university"></i></h1><hr>
    ...
    <span>Necesita el teléfono de la persona a enviar dinero</span><br><br>
    <form method="post">
    <div class="input-field">
          <input id="agregaso" name="agregaso" type="number" class="validate" required autocomplete="off">
          <label for="agregaso"><i class="fas fa-bolt"></i>&nbsp;Teléfono de la Cuenta&nbsp;<i class="fas fa-bolt"></i></label>
        </div>
        <div align="right">
        ...
        if(isset($_POST['pustup'])){
            $telfor = mysqli_real_escape_string($conexion, $_POST['agregaso']);
            if($telfor == $telesona){
                echo '<script> enviartuno(); </script>';exit;
            }
            $sql = "SELECT * FROM clientes WHERE telefono LIKE '$telfor'";
            $resultado = mysqli_query($conexion, $sql);
            if(mysqli_affected_rows($conexion)>0){
...
<div class="container">
<form method="post" action="./trashoy-tel.php">
...
<input type="hidden" name="tokenfor" id="telfor" value="<?php echo $telfor  ?>" readonly>

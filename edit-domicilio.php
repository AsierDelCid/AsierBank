...
<a href="./editar-cliente.php"><i class="fa-3x far fa-hand-point-left"></i></a></div>
    ...
    <?php
    $dni = mysqli_real_escape_string($conexion, $_POST['dnisupad3']);
    $nuevo1 = mysqli_real_escape_string($conexion, $_POST['domisiliado']);
    $nuevo2 = mysqli_real_escape_string($conexion, $_POST['domisiliadov2']);
    if($nuevo1 != $nuevo2){
        echo '<script> confirmaciono(); </script>';
        exit;
    }
    
    $sql = "UPDATE clientes SET domicilio='$nuevo1' WHERE dni LIKE '$dni'";
    $resultado = mysqli_query($conexion, $sql);
    if(mysqli_affected_rows($conexion)>0){

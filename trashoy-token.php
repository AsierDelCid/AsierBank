...
<body>
    ...
    if($promi == 0){
        if($flashmi == 0){
            ...
            if($anadir > 10000){
                header("location: ./cambia-plan.php");
                echo '<script> cambiaplan(); </script>';exit;
            }
            $interes = 5;
        }
    }else{
 ...
 $intereses = round($interes2);

 $cantint = $saldon - $intereses;
 $total = $anadir + $intereses;
    if($saldon <= 0){
    ...
    $sql = "UPDATE clientes SET saldo='$cantint' WHERE token LIKE '$tokenmi'";
    $resultado = mysqli_query($conexion, $sql);
    if(mysqli_affected_rows($conexion)>0){
    ...
    $saldon = $saldoa + $anadir;
    $sql = "UPDATE clientes SET saldo='$saldon' WHERE token LIKE '$tokenfor'";
    $resultado = mysqli_query($conexion, $sql);
    if(mysqli_affected_rows($conexion)>0){
    ...
    echo '<div class="row"><div class="col s2"><a id="dersasta" class="link" onclick="guardararch()">Guardar Recibo</a></div><div class="col s8"></div><div class="col s2><a href="" class="link" onclick="imprimirpantalla()">Imprimir Recibo</a></div></div>';
    echo '<h3 align="center">Recibo de la Transacción</h3><hr>';
    echo '<h5 align="center">Enviado a: &nbsp;<b><i class="fas fa-money-check-alt"></i>&nbsp;' . $tokenew . '**********</b></h5>';
    echo '<h5 align="center">Fecha y Hora: <b>' . $fecha . '</b></h5>';
    echo '<h5 align="center">Enviado por: <b>' . $tokenminew . '**********</b></h5>';
    ...
    echo '<h5 align="center">Estado: <b>Pagado de forma Online (<i style="color: #3b83bd;" class="fab fa-r-project"></i>)</b> <i class="fas fa-check" style="color: green"></i></h5>';
        ...
        echo '<h5 align="center">Estado: <b>Pagado de forma Online </b>&nbsp;<i class="fas fa-check" style="color: green"></i></h5>';
    }
